...
'''
#修改：上述代码中，循环、条件语句的使用过于频繁，对代码进行修改，减少对其的使用
#局部变量 repaired_code,在定义上存在一定的问题，建议定义为全局变量
'''
...
def get_vars_heuristics(code):
    ...
    '''
    #修改：局部变量 repaired_code,在定义上存在一定的问题
    '''
...
def revert_abbrev(line):
    ...
    return line
'''
#修改：上述代码存在重复的地方，可以使用循环
import re
def revert_abbrev(line):
    patterns = [
        (re.compile("(it|he|she|that|this|there|here)(\"s)", re.I), r"\1 is"),
        (re.compile("(?<=[a-zA-Z])\"s"), ""),
        (re.compile("(?<=s)\"s?"), ""),
        (re.compile("(?<=[a-zA-Z])n\"t"), " not"),
        (re.compile("(?<=[a-zA-Z])\"d"), " would"),
        (re.compile("(?<=[a-zA-Z])\"ll"), " will"),
        (re.compile("(?<=[I|i])\"m"), " am"),
        (re.compile("(?<=[a-zA-Z])\"re"), " are"),
        (re.compile("(?<=[a-zA-Z])\"ve"), " have")
    ]
...
'''
#修改：采用循环遍历输出,代码数量没什么变化，将引号的使用统一，统一使用双引号
query_list =["change row_height and column_width in libreoffice calc use python tagint",
             "What is the standard way to add N seconds to datetime.time in Python?",
             "Convert INT to VARCHAR SQL 11?",
             "python construct a dictionary {0: [0, 0, 0], 1: [0, 0, 1], 2: [0, 0, 2], 3: [0, 0, 3], ...,999: [9, 9, 9]}"
             ]

context_list = ["How to calculateAnd the value of the sum of squares defined as \n 1^2 + 2^2 + 3^2 + ... +n2 until a user specified sum has been reached sql()",
...
for context in code_list:
    print(python_context_parse(context))
for query in query_list:
    print(python_query_parse(query))
